    ...
    public class IrcConnection
    {
        ...
        private void _SimpleParser(object sender, ReadLineEventArgs args)
        {
            ...
            if (rawline[0] == ':') {
                ...
                int _replycode;
                if (!int.TryParse(messagecode, out _replycode))
                {
                    _replycode = (int)ReplyCode.Null;
                }
                replycode = (ReplyCode)_replycode;
                /* MLM: fixing all teh exceptions screwing with my debugging
                try {
        ...
        private class ReadThread
        {
            ...
            private void _Worker()
            {
                ...
                try {
                    string data = "";
                    try {
                        /*MLM: begin edit to allow for encodign failure
                        ...
                        while(_Connection.IsConnected)
                        {
                            try
                            {
                                data = _Connection._Reader.ReadLine();
                                if (data == null) break;
                            }
                            catch (IOException ioex)
                            {
                                //forward io exception
                                throw ioex;
                            }
                            catch (Exception)
                            {
                                //ignore all other exceptions (ie, text encoding failed)
                            }
                            /*MLM: end of edit to allow for encoding failure*/
                            _Queue.Enqueue(data);
#if LOG4NET
